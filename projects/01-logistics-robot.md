...
世界坐标系版本再加上陀螺仪世界角度，通过旋转矩阵把车体速度与世界速度互相转换，
外层叠加角度环控制。
</details>

<details>
<summary><b>3 自由度机械臂逆解是解析解吗？精度够吗？</b></summary>

是**工程降维**：只在 yz 平面解算大臂 + 小臂（平面 2R 解析解），
底座旋转（xy 平面）分离为独立可调变量。

这是为调试效率做的**主动取舍**——解耦后改一个动作只需调单个变量，
不用重解完整 3D 逆解；代价是末端定位需人工标定底座基准角，
且正解算（示教）在校赛阶段未实现。
</details>

<details>
<summary><b>为什么用五次多项式而不是三次？</b></summary>

五次可同时约束起点终点的**位置、速度、加速度**共 6 个边界条件，
保证加速度连续、jerk 有界，减少机械冲击并防止车轮打滑。
三次只能约束位置与速度，加速度在端点会跳变。
</details>

<details>
<summary><b>10 ms 路径队列怎么实现？</b></summary>

TIM5 定时中断维护系统时间戳（`sys_time_ms += 10`），
主循环每 10 ms 推演一次三轴队列，取该时刻的插值速度；
节点结构体带 `.task` 成员，可在指定节点执行自定义任务。
</details>

<details>
<summary><b>底盘与机械臂坐标系怎么统一？</b></summary>

陀螺仪 HWT101 提供世界坐标角度 `abs_degree_data`，
世界坐标系控制版本用它构造旋转矩阵，实现车体 ↔ 世界坐标转换。
</details>
